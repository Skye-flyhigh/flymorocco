...
import { getTourPricing, getCurrencyInfo, getAvailableCurrencies, calculateBookingTotal, type Currency } from "@/lib/utils/pricing";
...
export default function BookingForm(tour: TourSchedule) {
  ...
  const calculateTotal = () => {
    ...
    const soloCount = participants.filter(p => p.soloOccupancy).length + (mainBookerSolo ? 1 : 0);
  ...
  const updateParticipantCount = (newCount: number) => {
    ...
    if (newCount > participants.length) {
      ...
      for (let i = participants.length; i < newCount; i++) {
        newParticipants.push({ name: "", isPilot: false, soloOccupancy: false });
      }
  ...
  const updateParticipant = (index: number, updates: Partial<ParticipantData>) => {
    const newParticipants = [...participants];
  ...
  } else { // TODO: add a tour logic checking the availability of the tour so it can disable the button (isAvailable boolean!)
    return (
      <>
        <button
          className="btn btn-primary"
        ...
        >
          ...
          >
            <div className="container w-full mx-auto">
              ...
              <p className="font-extralight mb-6">{t("bookingForm.subtitle")}</p>
              
              {/* Currency Selection */}
              <div className="mb-6 p-4 bg-base-100 rounded-lg border border-base-300">
                <label className="block text-sm font-medium mb-3">Choose your preferred currency:</label>
                ...
                <div className="mt-3 text-sm text-base-content/70">
                  {(() => {
                    const pricing = getTourPricing(tour.type as TourSlug, selectedCurrency);
                    const currencyInfo = getCurrencyInfo(selectedCurrency);
                    
                    return (
                      <span>
                        {tour.type} tour: {currencyInfo.symbol}{pricing.base} per person
                        {pricing.solo > 0 && ` • Solo room: +${currencyInfo.symbol}${pricing.solo}`}
                      </span>
                    );
                  })()}
                </div>
              ...
              <fieldset className="fieldset mb-6">
                <legend className="fieldset-legend text-lg font-semibold mb-4">Main Contact</legend>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                  <div>
                    ...
                    />
                    {state?.errors?.name && (
                      <p id="name-error" aria-live="polite" className="text-error text-sm mt-1">
                        {state.errors.name}
                      </p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    />
                    {state?.errors?.email && (
                      <p id="email-error" aria-live="polite" className="text-error text-sm mt-1">
                        {state.errors.email}
                      </p>
                    )}
                  </div>
                ...
                <div className="flex gap-6 mb-4">
                  ...
                  <label className="flex items-center gap-2">
                    ...
                    <abbr title="Solo occupancy means you will have your own room.">
                      <span>Solo room (+{calculateTotal().symbol}{calculateTotal().soloPrice})</span>
                    </abbr>
                  </label>
                </div>
              </fieldset>
              ...
              <fieldset className="fieldset mb-6">
                <legend className="fieldset-legend text-lg font-semibold mb-4">Participants</legend>
                  ...
                  {participants.map((participant, index) => (
                    <div key={index} className="border border-base-300 rounded-lg p-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        ...
                        {participant.isPilot && (
                          <div>
                            <label className="fieldset-label">Pilot Email</label>
                            <input
                              type="email"
                      ...
                      <div className="flex gap-6">
                        <label className="flex items-center gap-2">
                          <input
                            ...
                            onChange={(e) => updateParticipant(index, { 
                              ...
                              email: e.target.checked ? participant.email : undefined
                            })}
                            className="checkbox"
                            disabled={isPending}
                          />
                        ...
                        <label className="flex items-center gap-2">
                          <input
                            ...
                            onChange={(e) => updateParticipant(index, { soloOccupancy: e.target.checked })}
                            className="checkbox"
                            disabled={isPending}
                          />
                          <span>Solo room (+{calculateTotal().symbol}{calculateTotal().soloPrice})</span>
                  ...
                  return (
                    <div className="space-y-2">
                      <div className="flex justify-between">
                        <span>{participantCount + 1} participant{participantCount > 0 ? 's' : ''} × {pricing.symbol}{pricing.basePrice}</span>
                        <span>{pricing.symbol}{pricing.baseTotal.toLocaleString()}</span>
                      </div>
                      
                      {pricing.soloCount > 0 && (
                        <div className="flex justify-between">
                          <span>{pricing.soloCount} solo room{pricing.soloCount > 1 ? 's' : ''} × {pricing.symbol}{pricing.soloPrice}</span>
                          <span>{pricing.symbol}{pricing.soloTotal.toLocaleString()}</span>
                        </div>
                      )}
                      ...
                      <div className="flex justify-between text-xl font-bold">
                        <span>Total</span>
                        <span>{pricing.symbol}{pricing.grandTotal.toLocaleString()}</span>
                      </div>
                      
                      <div className="text-sm text-base-content/70 flex justify-between">
                        <span>Currency</span>
                        <span>{pricing.currency} - {pricing.currencyInfo.name}</span>
                      </div>
                      
                      <p className="text-sm text-base-content/70 mt-2">
                        Price includes 1 week guided tour with transportation,accommodation and meals.
                      </p>
                    </div>
                  );
                })()}
