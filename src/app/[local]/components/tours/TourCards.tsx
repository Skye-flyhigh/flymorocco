        ...
        <section id="tour-cards" className="py-20 px-10">
            ...
            <div id="card-container" className="flex flex-wrap gap-4 justify-center">
                ...
                tours.map((tour) => {
                    ...
                    if(!height || !width) console.warn("Tour Cards: unable to extract the width/height of image card.")

                    return (
                    <article key={tour.type} className="card bg-base-100 w-96 shadow-sm hover:bg-base-200 hover:shadow-xl transition-all">
                        <figure>
                            <Image 
                                src={tour.img}
                                alt={tour.alt}
                        ...
                        <div className="card-body">
                            <h2 className="card-title">{t(`toursDetails.${tour.type}.focus`)}</h2>
                            <p>{t(`toursDetails.${tour.type}.note`)}</p>
                            <div className="card-actions justify-end">
      <Link href={`/tours/${tour.type}`}><button type="button" className="btn btn-primary">{t("view")}</button></Link>
    </div>
                        </div>
                    </article>
                )})
