...
export default function FeaturedSites() {
  ...
  return (
    <section id="featured-site" className="sm:grid grid-cols-2 bg-base-200 shadow-md">
      <div id="cell" className="flex flex-col justify-evenly p-10 my-10">
      <h2 className="section-title">{t("featuring")}</h2>
      <h3 className="section-subtitle capitalize">{t(`${featuredSite.slug}.name`)}</h3>
      <p className="prose">{t(`${featuredSite.slug}.description`)}</p>
      <a href={`/site-guides/${featuredSite.slug}`} className="btn btn-primary self-end max-w-64 mt-6">{t(`${featuredSite.slug}.name`)}</a>
      </div>
