  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: {
        ...
        tourType: bookingData.tourType,
        participantCount: totalPeople.toString(),
        participantNames: bookingData.participants
          .map((p) => p.name)
          .join(", "),
        pilotEmails: bookingData.participants
          .filter((p) => p.isPilot && p.email)
          .map((p) => `${p.name}:${p.email}`)
          .join(", "),
        soloRooms: bookingData.participants
          .filter((p) => p.soloOccupancy)
          .map((p) => p.name)
          .join(", "),
        totalPilots: (
          bookingData.participants.filter((p) => p.isPilot).length +
          (bookingData.isPilot ? 1 : 0)
        ).toString(),
