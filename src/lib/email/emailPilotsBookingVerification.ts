...
import { createPilotVerificationEmail, createPilotWelcomeEmail } from "./templates/pilotEmails";
...
export async function emailPilotsBookingVerification(
  ...
  try {
    ...
    await Promise.all(pilots.map(async (pilot) => {
      try {
        const emailTemplate = emailType === "verification" 
        ...
        await resend.emails.send({
          ...
          to: pilot.email,
          subject: emailType === "verification" 
